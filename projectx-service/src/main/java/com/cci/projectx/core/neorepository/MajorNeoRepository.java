...
public interface MajorNeoRepository extends GraphRepository<MajorNeo> {
    ...
    //根据用户和专业名称查找二度人脉
    @Query("MATCH (p:User { userId:{0}})-[r:FRIEND]->(m:User)-[a:FRIEND]->(b:User),(b)-[s:MAJOR]->(c:Major{name:{1}}) return b")
    Collection<UserNeo> getConnecTwoUserFromName(Long userId, String majorName);
    //根据用户和专业名称查找一度度人脉
