...
public interface CompanyNeoRepository extends GraphRepository<CompanyNeo> {
    ...
    //根据用户和公司名称查找二度人脉
    @Query("MATCH (p:User { userId:{0}})-[r:FRIEND]->(m:User)-[a:FRIEND]->(b:User),(b)-[s:COMPANY]->(c:Company{name:{1}}) return b")
    Collection<UserNeo> getConnecTwoUserFromName(Long userId, String companyName);
    //根据用户和公司名称查找一度度人脉
