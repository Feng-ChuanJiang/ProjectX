...
public interface DepartmentNeoRepository extends GraphRepository<DepartmentNeo> {
    ...
    //根据用户和部门名称查找二度人脉
    @Query("MATCH (p:User { userId:{0}})-[r:FRIEND]->(m:User)-[a:FRIEND]->(b:User),(b)-[s:DEPARTMENT]->(c:Department{name:{1}}) return b")
    Collection<UserNeo> getConnecTwoUserFromName(Long userId, String departmentName);
    //根据用户和部门名称查找一度度人脉
