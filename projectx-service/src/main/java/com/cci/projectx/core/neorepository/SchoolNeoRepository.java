...
public interface SchoolNeoRepository extends GraphRepository<SchoolNeo> {
    ...
    //根据用户和学校名称查找二度人脉
    @Query("MATCH (p:User { userId:{0}})-[r:FRIEND]->(m:User)-[a:FRIEND]->(b:User),(b)-[s:SCHOOL]->(c:School{name:{1}}) where b.userId<>p.userId  return b.userId as userId,m.userId as fridId")
    Collection<Map<String,Object>> getConnecTwoUserFromName(Long userId, String schoolName);
    //根据用户和学校名称查找一度度人脉
